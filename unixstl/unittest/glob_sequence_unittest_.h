...
namespace unittest
{
    namespace
    {
        namespace unixstl_glob_sequence_item_comparator
        {
            struct equal_path
            {
            public:
    ...
    defined(_WIN32)
...

            {
                ...
                if( !std::equal(gs1.begin(), gs1.end(), gs2.begin(), unixstl_glob_sequence_item_comparator::equal_path()) ||
                    !std::equal(gs2.begin(), gs2.end(), gs1.begin(), unixstl_glob_sequence_item_comparator::equal_path()))
                {
                    r->report("implicit directory instance not equal to explicit current directory instance", __LINE__);
                    bSuccess = false;
                }

                if( !std::equal(gs3.begin(), gs3.end(), gs4.begin(), unixstl_glob_sequence_item_comparator::equal_path()) ||
                    !std::equal(gs4.begin(), gs4.end(), gs3.begin(), unixstl_glob_sequence_item_comparator::equal_path()))
                {
